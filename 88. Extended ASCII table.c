...
void print_out_header();
void print_out_line(const int index);
void iterate_over_range_of_chars(const int start, const int end);

int main() {
    int start_of_the_column = 32, end_of_column = 63;
    print_out_header();
    iterate_over_range_of_chars(start_of_the_column, end_of_column);
    return 0;
}

void iterate_over_range_of_chars(const int start, const int end) {
    for (int index = start; index <= end; ++index) {
        print_out_line(index);
    }
}

void print_out_line(const int index) {
    int size_of_column = 32, max_diff = size_of_column * 6;
    printf("|  ");
    for (int temp_index = index; temp_index <= index + max_diff; temp_index += size_of_column) {
        printf("%3c%4d%#5x  |", temp_index, temp_index, temp_index);
    }
    printf("\n");
}
